...
<body>
    ...
    <h1 class="text-center text-danger m-3 "> Upadte User</h1>
    <form class="container" method="post" action="" enctype="multipart/form-data">
        <div class="mb-3">
            <label for="exampleInputName" class="form-label">Name</label>
            <input type="text" class="form-control" id="exampleInputEmail1" value="<?php echo $result['username']?>" aria-describedby="nameHelp" required name="username">
            
        </div>
        <div class="mb-3">
            <label for="exampleInputEmail1" class="form-label">Email address</label>
            <input type="email" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" value="<?php echo $result['email']?>" required name="email">
        </div>
        <div class="mb-3">
            <label for="exampleInputPassword1" class="form-label">Password</label>
            <input type="password" class="form-control" id="exampleInputPassword1" value="<?php echo $result['password']?>" required name="password">
        </div>
        <div class="mb-3">
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control" id="exampleInputRoomNo" value="<?php echo $result['RoomNo']?>" required name="rommno">
        </div>
        <div class="mb-3">
            <label for="exampleInputExt" class="form-label">EXT.</label>
            <input type="text" class="form-control" id="exampleInputExt" value="<?php echo $result['Ext']?>" required name="ext">
        </div>
        <div class="mb-3">
            <label for="formFile" class="form-label">Profile Picture</label>
            <input class="form-control" type="file" id="formFile" value="<?php echo $result['profilepicture']?>" required name="profilepicture">
        </div>
...
//             }
    ...
    $query="UPDATE `users` set username='$username',email='$email',password='$password',Ext='$ext',image='$folder',RoomNo='$rommno' WHERE id='$id'";
    $runQuery=mysqli_query($con,$query);
    if($runQuery){
